...
class SHA1Library
{
		...
		for($i = 0; $i < $count; $i += 16)
		{
			...
			for($j = 0; $j < 80; $j++)
			{
				if ($j < 16) {
					...
					//echo (int)($w[$j-3] ^ $w[$j-8] ^ $w[$j-14] ^ $w[$j-16])."\r\n";
	...
	function str2binb($str)
	{
		$bin = array();
		$mask = (1 << $this->chrsz) - 1;
		$times = strlen($str) * $this->chrsz;
		for($i = 0; $i < $times; $i += $this->chrsz) {
	...
	function binb2str($bin)
	{
		...
		*/
		$bin = $this->binb2hex($bin);
		$len = strlen($bin);
		$str = '';
		for ($i=0;$i<$len;$i+=2) {
			$str .= chr(hexdec($bin[$i].$bin[$i+1]));
		}
		return $str;
	}
